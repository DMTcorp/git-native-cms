...
function memoryRuntime(): HostedCmsRuntime {
  ...
  const server = createCmsServer({
    ...
    queries: {
      ...
      getDocument: async (_changeId, documentId) =>
        content.readDocument({ ref: sandboxChange.branchName, documentId }),
      listReleases: async () => [],
  ...
  return {
    ...
    editorState: async () => ({
      ...
      baseDocument: sandboxDocument,
      productionDocument: sandboxDocument,
      documents: (await content.listDocuments({ ref: sandboxChange.branchName })).items,
      contentDocuments: [sandboxDocument],
      previewDocument: sandboxDocument,
      assets: [],

...
async function chunksFor(path) {
  ...
  const chunks = [
    ...new Set(
      Object.values(manifest.value.clientModules ?? {}).flatMap(
    ...
    chunks
      ...
      .map(async (chunk) => ({
        ...
        source: await readFile(
          new URL(chunk.replace(/^\/_next\//, ""), nextRoot),
          "utf8",
        ),
      })),
  );
...
const publicManifest = await routeManifest(
...
const editorLeak = publicChunks.find(
  ({ source }) =>
    source.includes("cms-editor-shell") ||
    source.includes("@git-native-cms/editor"),
);
...
process.stdout.write(
  `Bundle budgets passed: public editor runtime 0 B, editor ${editorBytes} B gzip, bridge ${bridgeBytes} B gzip.\n`,
);
